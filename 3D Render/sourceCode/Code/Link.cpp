...
Link::Link(const float& width, const float& height, const std::string& filename)
	: width(width), height(height), _time(0.0), runAnimate(true), dx(4.0f)
{
...
void Link::Update(const double& deltaTime)
{
	...
	if (runAnimate)
	{
		pos[0] -= (1.0f + scale[0])*dx*static_cast<float>(deltaTime)*9;
        pos[1] -= (1.0f + scale[1])*dx*static_cast<float>(deltaTime)*5;
        if (pos[0] < -scale[0]*70.0f){
            pos[0] = 100*i; pos[1] = 50*j;
        }

		if (_time > 0.1)
		{
			spriteFrame = spriteFrame > 6 ? 0 : spriteFrame + 1; // shift frame up by 1 (wrap at 7)
			_time = 0.0; // reset frame counter
		}
	}
