...
Sphere::Sphere(const std::string& filename) : Sphere()
{
    SetTexture(filename);
}

void Sphere::SetTexture(const std::string& filename)
{
...
void Sphere::SubDivide(int recurse_idx, float *a, float *b, float *c)
{
...
void Sphere::DrawFace(float *a, float *b, float *c)
{
    ...
    // vertex a
        glTexCoord2f(sa, ta);
        glNormal3f(a[0], a[1], a[2]); glVertex3fv(a);
    // vertex b
        glTexCoord2f(sb, tb);
        glNormal3f(b[0], b[1], b[2]); glVertex3fv(b);
    // vertex c
        glTexCoord2f(sc, tc);
        glNormal3f(c[0], c[1], c[2]); glVertex3fv(c);
    
    
    glEnd();
}

void Sphere::FixSeam(float &a, float &b, float &c)
{
...
void Sphere::HandleKey(unsigned char key, int state, int mx, int my)
{
    // Ignore key-release
    if (!state) return;
    // Handle key
